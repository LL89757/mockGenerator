...
const mockGenerator = {
  async init({ swaggerUrl, outputPath, dataLength, fileName, ignoreList, outputFileType }) {
    ...
    let swaggerData = await this.parse();
    if (swaggerData.length === 0) {
      signale.error(`swaggerData cannot be empty`);
      return;
    }
    let mockData = this.getMockData(swaggerData);
    await this.generateMockFile(mockData);
    signale.complete('mock generate success');
  },
  //   解析swagger-api-doc
  async parse() {
    return new Promise(async (resolve, reject) => {
      try {
        const { paths } = await swaggerParserMock(this.swaggerUrl);
        if (!paths) {
          resolve([]);
          return;
        }
...
`;
  ...
  //   生成mock文件
  async generateMockFile(mockData) {
    await this.makeDir();
    for (let key in mockData) {
      await this.writeMockFile(mockData[key], key);
    }
  },
  async writeMockFile(data, fileName) {
    // 写入文件
    let temp = '';
...
module.exports = {
  ${data}};`;
    ...
    fs.writeFile(`${this.outputPath}/${fileName}.${this.outputFileType}`, temp, 'utf-8', err => {
      if (err) {
        signale.fatal(error);
        throw err;
      }
  ...
  async makeDir() {
    return new Promise((resolve, reject) => {
      fs.exists(this.outputPath, exists => {
        if (!exists) {
          console.log(this.outputPath,111)
          // 新建该文件夹
          mkdirp(this.outputPath).then((made)=>{
            console.log(made)
            resolve();
          });
        } else {
